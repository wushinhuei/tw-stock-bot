...
async function main() {
  ...
  const audit = {
    ...
    sourceOrder: ['TWSE_MCP_OR_MOPS_MCP', 'YAHOO_FINANCE_MCP_SUPPLEMENT'],
    derivedProducts: [],
    overwriteRule: 'Yahoo Finance MCP must never overwrite official TWSE/MOPS rows', results, errors
  };
  const filename = `mcp_daily_sync_${date}.json`;
  const saved = await writer.upsertText(filename, `${JSON.stringify(audit, null, 2)}\n`);
  await writer.upsertText('manifest.json', `${JSON.stringify({ schemaVersion: 5, generatedAt: audit.generatedAt, latestDate: date, latestFile: filename, driveFileId: saved.id, ok, policyMode: POLICY.mode, sources: ['TWSE_MCP', 'MOPS_MCP', 'YAHOO_FINANCE_MCP'], derivedProducts: [] }, null, 2)}\n`);
