...
function runResultStatus(report) {
  process.stderr.write('\n[q2-pipeline] MATERIALIZE formal Q2 result status\n');
  const result = run('node', ['cloud_simulator/scripts/q2_result_status.js']);
  report.push({ name: 'Formal Q2 result status', ...result });
  return result;
}
...
function main() {
  ...
  process.stderr.write('\n[q2-pipeline] PROBE strict intraday source\n');
  const probe = run('node', ['cloud_simulator/scripts/probe_q2_intraday_source.js']);
  report.push({ name: 'Strict intraday source probe', ...probe });

  if (process.env.SKIP_INTRADAY !== '1') {
    if (process.env.SJ_API_KEY && process.env.SJ_SEC_KEY) {
      process.stderr.write('\n[q2-pipeline] START intraday fallback backfill (TWSE public MCP has no per-stock minute history)\n');
      ...
      report.push({ name: 'Intraday fallback backfill', ok: false, status: null, provider: null, blocker: 'NO_STRICT_HISTORICAL_INTRADAY_SOURCE_AVAILABLE' });
      process.stderr.write('[q2-pipeline] No complete historical minute archive and no configured fallback acquisition source. Daily-only approximation is forbidden.\n');
    }
  }
  ...
  const finalStatus = runResultStatus(report);
  if (!finalStatus.ok) {
    process.exitCode = 2;
    return;
  }
...
module.exports = { run, runRequired, runResultStatus };
