...
function main() {
  ...
  // These two gates run before any data/replay work. They prevent silent provider drift
  // and prevent changing the strategy after seeing the Q2 result.
  ...
  if (process.env.SKIP_INTRADAY !== '1') {
    if (process.env.SJ_API_KEY && process.env.SJ_SEC_KEY) {
      ...
      report.push({
        name: 'Intraday fallback backfill', ok: false, status: null,
        provider: 'SHIOAJI_ONLY_BECAUSE_TWSE_MINUTE_HISTORY_UNAVAILABLE',
        blocker: 'SJ_API_KEY/SJ_SEC_KEY_NOT_SET'
      });
  ...
  // Re-check the frozen strategy after all potentially long-running data acquisition steps.
  ...
  process.stdout.write(`${JSON.stringify({
    status: 'READY_FOR_STRICT_REPLAY',
    policy: {
