...
test('Taiwan financial news MCP suppresses ordinary news outside Top50', async () => {
...
test('Taiwan financial news MCP keeps Top50-related news metadata', async () => {
  const service = new TaiwanNewsMcp({
  ...
  assert.equal(result.rows[0].reason, 'TOP50_RELATED');
});
