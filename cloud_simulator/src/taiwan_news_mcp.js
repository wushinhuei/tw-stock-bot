...
const SOURCES = Object.freeze({
  UDN_ECONOMIC_DAILY: Object.freeze({
    source: '經濟日報',
    host: 'money.udn.com',
    automatedMethods: Object.freeze(['LICENSED_API', 'RSS'])
  }),
  CTEE: Object.freeze({
    source: '工商時報',
    host: 'ctee.com.tw',
...
function normalizeItem(item, sourceConfig) {
  ...
  const normalized = {
    ...
    acquisitionMethod: String(item?.acquisitionMethod || 'LICENSED_API').toUpperCase(),
    relatedSymbols,
    top100Related: item?.top100Related === true,
    marketScope: item?.marketScope || item?.scope || null,
...
async function fetchJsonEndpoint(name, endpoint, options = {}) {
  ...
  const response = await (options.fetchImpl || fetch)(status.endpoint, {
    headers: {
      ...
      'user-agent': 'tw-stock-taiwan-news-mcp/1.0',
  ...
  const rows = deduplicateNews(rawRows.map(row => normalizeItem(row, config))).filter(row => row.eligible);
  return {
    source: config.source,
    status: 'OK',
    rows,
    suppressedCount: rawRows.length - rows.length,
...
class TaiwanNewsMcp {
  constructor(options = {}) {
    ...
    this.endpoints = {
      UDN_ECONOMIC_DAILY: options.udnEndpoint || process.env.UDN_ECONOMIC_DAILY_NEWS_ENDPOINT || '',
      CTEE: options.cteeEndpoint || process.env.CTEE_NEWS_ENDPOINT || ''
    };
    this.tokens = {
      UDN_ECONOMIC_DAILY: options.udnToken || process.env.UDN_ECONOMIC_DAILY_NEWS_TOKEN || '',
      CTEE: options.cteeToken || process.env.CTEE_NEWS_TOKEN || ''
    };
  }
  ...
  listTools() {
    return {
      tools: [
        { name: 'taiwan_financial_news', description: 'Read licensed Taiwan financial news metadata from Economic Daily News and Commercial Times, restricted to Top100-related or major global events.', inputSchema: { type: 'object', properties: { source: { type: 'string', enum: ['ALL', 'UDN_ECONOMIC_DAILY', 'CTEE'] } } } },
  ...
  async callTool(name, args = {}) {
    if (name === 'taiwan_financial_news_sources') {
      return { structuredContent: { rows: Object.entries(SOURCES).map(([key, config]) => ({ key, source: config.source, configured: Boolean(this.endpoints[key]), licensedOnly: true })) } };
    }
    ...
    const results = await Promise.allSettled(keys.map(key => fetchJsonEndpoint(key, this.endpoints[key], { fetchImpl: this.fetchImpl, token: this.tokens[key] })));
    const rows = [];
    const sources = [];
    const errors = [];
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        sources.push({ key, ...result.value, rows: undefined });
        rows.push(...(result.value.rows || []));
      } else errors.push({ key, error: String(result.reason) });
    });
    return { structuredContent: { status: errors.length ? 'PARTIAL' : 'OK', rows: deduplicateNews(rows), sources, errors, policy: 'TOP100_RELATED_OR_GLOBAL_MAJOR_ONLY', licensedOnly: true } };
  }
}
