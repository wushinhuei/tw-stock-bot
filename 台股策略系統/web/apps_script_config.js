...
// 健康狀態只阻擋即時 MCP 與必要稽核；Drive歷史、20季與新聞不再阻擋交易。
(function installGlobalDataHealth() {
  ...
  function checkedAt(value){if(!value)return '-';const date=new Date(value);return Number.isNaN(date.getTime())?'-':new Intl.DateTimeFormat('zh-TW',{timeZone:'Asia/Taipei',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(date);}
  async function refresh(){const node=mount();if(!node)return;node.textContent='全系統資料：檢查中…';try{const response=await fetch(`${endpoint()}?t=${Date.now()}`,{cache:'no-store'});const data=await response.json();const complete=data?.ok===true&&data?.status==='COMPLETE';const failed=Array.isArray(data?.failedChecks)&&data.failedChecks.length?`；異常：${data.failedChecks.join('、')}`:'';const inspected=`｜資料檢查時間 ${checkedAt(data?.generatedAt)}`;node.textContent=complete?`全系統資料：✅ 完整｜最後完整交易日 ${data.latestCompleteTradeDate||'-'}${inspected}｜每日完整性檢查已通過`:`全系統資料：⚠️ ${data?.status||'UNKNOWN'}｜最後完整交易日 ${data?.latestCompleteTradeDate||'-'}${inspected}${failed}｜未通過時禁止以不完整資料新增交易`;node.style.background=complete?'#eef8f2':'#fff4e5';node.style.color=complete?'#17633d':'#8a5500';}catch(error){node.textContent='全系統資料：⚠️ 無法取得完整性檢查結果；視為未通過。';node.style.background='#fff4e5';node.style.color='#8a5500';}}
