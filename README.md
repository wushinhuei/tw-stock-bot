...
盤中網頁會約每 15 秒呼叫 Apps Script `action=refresh`，主動重抓 TWSE MIS 報價；盤後則每 1 分鐘讀取已儲存的最新結果。Apps Script 會用台北時間與證交所休市日資料判斷是否需要執行，週末、休市日與非更新時段會直接回傳快取，避免浪費用量。模擬買進以賣一價估算，賣出與持倉估值以買一價估算。收盤後若證交所 BFT41U 盤後定價資料已發布，系統會額外執行一次盤後定價模擬，並以盤後成交價估算買進、賣出與持倉價值。
